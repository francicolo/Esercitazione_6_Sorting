...
{
    //il main prende in input un intero che sarà la size dei vettori v1, v2, v3
    ...
    else //se non viene messo un input, m assume un valore di default
        cerr << "Use default value: "  << m << endl;

    //Definisco il vettore v1
    vector<int> v1(m);
    std::iota(v1.begin(), v1.end(), -4);

    //Definisco il vettore v2
    srand(2);
    vector<double> v2(m);
